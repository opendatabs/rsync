...
CMD ["python3", "-m", "rsync.sync_files"]

# Docker commands to create image and run container:
# docker build --build-arg SERVER_NAMES=server1,server2,server3 -t rsync .
# docker run -it --rm -v /home/syncuser/.ssh/id_rsa:/root/.ssh/id_rsa:ro -v /data/dev/workspace:/code --name rsync rsync python3 -m rsync.sync_files example_config_file.json
